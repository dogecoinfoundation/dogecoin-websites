<!-- METADATA
title: TeamSeas Campaign
subtitle: Charitable
description: The Dogecoin Foundation and community collaborated with MrBeast in 2021 on the #TeamSeas campaign (also known as CleanSeas). The initiative aimed to remove trash from oceans, rivers, and beaches.
summary: The Dogecoin Foundation and community collaborated with MrBeast in 2021 on the #TeamSeas campaign (also known as CleanSeas). The initiative aimed to remove trash from oceans, rivers, and beaches.
keyPoints:
- The goal was to raise funds to help remove 30 million pounds of trash from oceans
- Each $1 donated removed 1 pound of trash
- The Dogecoin community participated by donating DOGE
- The campaign partnered with Ocean Conservancy and The Ocean Cleanup
-->

# TeamSeas Campaign

## Campaign Details

The #TeamSeas campaign was a massive collaborative effort to clean up our oceans and waterways. The Dogecoin community played a significant role in supporting this environmental initiative.

## Community Impact

The campaign showcased the power of the Dogecoin community to come together for environmental causes, demonstrating that cryptocurrency can be a force for positive change in the world.